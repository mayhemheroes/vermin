  ...
  def visit_keyword(self, node):
    for func_name in self.__s.function_name_stack:
      ...
      if func_name in self.__s.import_mem_mod:
        ...
        self.__add_kwargs(dotted_name([mod, func_name]), node.arg, self.__s.line)
      ...
      # Lookup indirect names via variables.
        ...
        if res in self.__s.import_mem_mod:
          mod = self.__s.import_mem_mod[res]
          self.__add_kwargs(dotted_name([mod, res, exp_name[1:]]), node.arg, self.__s.line)

        # Try as FQN.
        else:
          self.__add_kwargs(dotted_name([res, exp_name[1:]]), node.arg, self.__s.line)

      # Only add direct function if not found via module/class/member.
      else:
        self.__add_kwargs(func_name, node.arg, self.__s.line)
      ...
      def is_none_or_name(node):
        return is_none_node(node) or \
          (isinstance(node, ast.Name) and
           ((node.id not in self.__s.name_res and node.id not in self.__s.user_defs) or
            (node.id in self.__s.name_res_type)))
      ...
      def is_union_type(node):
        return isinstance(node, ast.Name) and \
           ((node.id not in self.__s.name_res and node.id not in self.__s.user_defs) or
            ...
            (node.id in self.__s.name_res_type and len(self.__s.name_res_type[node.id]) > 1) or
            # Unless it's the target value, then require not name res, is user def and res type.
            (node.id not in self.__s.name_res and node.id in self.__s.user_defs and \
             node.id in self.__s.name_res_type))

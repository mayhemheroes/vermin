 ...
 *
 * Why a compiled wrapper rather than calling `python3 ...` directly from
 ...
 * sparing /usr/bin, /bin, ... Since the CPython interpreter may live under a
 * spared system path, a test.sh that shells straight to the system python would
 * run identically under sabotage and be flagged reward-hackable.
 *
 * Routing the suite through this NON-system binary makes the neuter bite: under
 * the shim this wrapper is `_exit(0)`'d before it can exec python, so the suite
 * produces no results and the oracle is correctly detected as behavioral.
 *
 * argv forwarded to: python3 <forwarded args...>
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
